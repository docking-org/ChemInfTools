...
void main(int argc, char *argv[]) {
  ...
  int maxclusters; 
   ...
   if( argc == 5 ) {
      ...
      printf("(2) smiles file.\n");
      printf("(3) tanimoto coefficient threshold to define clustering. values between 0 and 1. smaller values will result in larger clusters. recommend value of 0.5\n");
      printf("(4) max number of clusters,  the program will terminate once this number is reached.\n  If you want to process the whole list than make this number grater than the number of molcules.\n");
      exit(1);
   }
  ...
  strcpy(filename_smi, argv[2]);
  sscanf(argv[3], "%lf", &tc_threshold);
  sscanf(argv[4], "%d", &maxclusters);
  ...
  fingerprints_binary = (uint16_t *)malloc(fpbsize*size * sizeof(uint16_t));
  ...
  fingerprints_one_count = (int *)malloc(size * sizeof(int *));
